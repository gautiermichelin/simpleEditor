 ...
 */
	...
	class SimpleEditorController extends SimpleEditorBaseController {
		...
		protected $opn_record_id;		// name of "subject" table (what we're editing)
		...
		public function Add($pa_options = null) {
			$vn_record_id=$this->request->getParameter($this->opn_record_id, pInteger);
			if($vn_record_id) {
				$url = "/".str_ireplace("/Add/", "/Edit/", $this->getRequest()->getRequestUrl()."/".$this->opn_record_id."/".$vn_record_id);
				$this->redirect($url);
			} else {
				...
				$vt_record->set(array('access' => 2, 'status' => 3, 'idno' => '','type_id' => $vn_type_id));//Define some intrinsic data.
				$vt_record->insert();//Insert the record
				if($vt_record->numErrors()) {
					var_dump($vt_record->getErrors());
					die("Oups. Erreur.");
				}
				$url = "/".str_ireplace("/Add/", "/Edit/", $this->getRequest()->getRequestUrl()."/".$this->opn_record_id."/".$vt_record->getPrimaryKey());
				$this->redirect($url);
			}
		}
		...
		public function Save($pa_values=null, $pa_options=null) {
			...
			if (((int) $_COOKIE["simpleEditor".$this->ops_table_propername."ControllerLastEdited"]>0) && (!$vn_record_id)) {
				...
				$o_data = new Db();
				$vs_query = "SELECT ".$this->opn_record_id." FROM ".$this->ops_table_name." WHERE deleted = 0 AND ".$this->opn_record_id." = ?";
			...
			if($vn_record_id) {
				...
				$qr_result = $o_data->query("
					    SELECT min(".$this->opn_record_id.") as ".$this->opn_record_id."
					    FROM ".$this->ops_table_name." 
					    WHERE deleted = 0
					 ");

				while($qr_result->nextRow()) {
					if ($qr_result->get($this->opn_record_id)) {
						$url = "/".str_ireplace("//","", str_ireplace("/".$this->opn_record_id."/$vn_record_id","",$vs_request_url))."/".$this->opn_record_id."/".$qr_result->get($this->opn_record_id);
						break;
					}
				}
			} else {
				//die("cookie valide");
				$url = "/".str_ireplace("//","", str_ireplace("/".$this->opn_record_id."/$vn_record_id","",$vs_request_url))."/".$this->opn_record_id."/".$vn_lastedited_record_id;
			}
			if($url) {
				//var_dump($url);
				print "<script>
        document.location.href=\"".$url."\";
			...
			if ($vn_record_id) {
				$this->view->setVar($this->opn_record_id, $vn_record_id);
