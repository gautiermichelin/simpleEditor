...
class StorageLocationsController extends SimpleEditorBaseController {
	# -------------------------------------------------------
	...
	public function __construct(&$po_request, &$po_response, $pa_view_paths=null) {
		...
		$this->ops_table_name="ca_storage_locations";
	...
	public function Edit($pa_values=null, $pa_options=null) {
		$vn_subject_id=$this->request->getParameter('location_id', pInteger);
		if($vn_subject_id) {
			// Setting cookie of last edited object
			setcookie("simpleEditorStorageLocationsControllerLastEdited", $vn_subject_id, time()+3600*24*30,"/");
		} elseif ((int) $_COOKIE["simpleEditorStorageLocationsControllerLastEdited"]>0) {
			$vn_subject_id = $_COOKIE["simpleEditorStorageLocationsControllerLastEdited"];
			$url = "/".str_ireplace("//","", str_ireplace("/location_id/","",$this->getRequest()->getRequestUrl()))."/location_id/".$vn_subject_id;
			$this->redirect($url);
		} else {
		...
		$va_nav = $t_ui->getScreensAsNavConfigFragment($this->request, caGetDefaultItemID("storage_location_types"), $this->request->getModulePath(), $this->request->getController(), $this->request->getAction(),
			array(),
			array(),
			false,
		...
		if ($vn_subject_id) {
			...
			$vt_item = new ca_storage_locations($vn_subject_id);
	...
	public function Save($pa_options=null) {
		...
		if ($vn_subject_id) {
			$this->view->setVar('location_id', $vn_subject_id);
			$vt_item = new ca_storage_locations($vn_subject_id);
			$this->view->setVar('t_item', $vt_item);
			$vt_representations = $vt_item->getRepresentations(array('preview170','medium'));
			$this->view->setVar('representations', $vt_representations);
	...
	public function Add($pa_options = null) {
		$vn_subject_id=$this->request->getParameter('location_id', pInteger);
		if($vn_subject_id) {
			$url = "/".str_ireplace("/Add/", "/Edit/", $this->getRequest()->getRequestUrl()."/location_id/".$vn_subject_id);
			$this->redirect($url);
		} else {
			$vn_type_id=$this->request->getParameter('type_id', pInteger);
			if(!$vn_type_id) {
				...
				$vn_type_id = caGetDefaultItemID("storage_location_types");
			}
			$vt_item = new ca_storage_locations();
			...
			$url = "/".str_ireplace("/Add/", "/Edit/", $this->getRequest()->getRequestUrl()."/location_id/".$vt_item->getPrimaryKey());
			$this->redirect($url);
		}
	}
