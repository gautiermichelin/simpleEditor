...
//$vt_representations = $t_object->getRepresentations(array('preview170','medium'));
	...
	<div id="simple_editor_top">
		<div id="top_box">
				...
				if (sizeof($va_reps) > 0) {
					...
					if($va_img_primary) {
						$vs_buf .= "<div id=\"simple_editor_main_img\" class=\"simple_editor_img_primary\" style=\"background-image:url($va_img_primary);background-size:contain;background-position:50% 50%;background-repeat:no-repeat;\" onclick='caMediaPanel.showPanel(\"".__CA_URL_ROOT__."/index.php/editor/objects/ObjectEditor/GetRepresentationEditor/object_id/".$vn_subject_id."/representation_id/".$va_img_primary_representation_id."\"); return false;' ><span class=\"helper\"></span></div>";
...
// index.php/editor/objects/ObjectEditor/GetRepresentationEditor/object_id/".$vn_subject_id."/representation_id/".$va_img_primary_representation_id
					...
					if (sizeof($va_imgs)) {
						foreach($va_imgs as $va_img) {
							$vs_imgs_buf .= "<span class='image-cell'><img src=".$va_img."></span>";
						}
			...
			<div id="top_editor_box" style="<?php print (sizeof($va_reps) > 0 ? "width:65%" : "width:100%")?>">
				<div class="control-box rounded" id="topButtons">
					<a href="#" class="form-button 1457282139" onclick='jQuery("#topform").submit();' id="<?php print $vs_form_id; ?>_submit"><span class="form-button"><img
					...
					<?php if ($vn_subject_id): ?>
						<a href="<?php print __CA_URL_ROOT__; ?>/index.php/simpleEditor/<?php print $vs_simpleEditor_controller; ?>/Edit/<?php print $vs_last_selected_path_item; ?>/object_id/<?php print $vn_subject_id; ?>"
						...
						<a
							href="<?php print __CA_URL_ROOT__; ?>/index.php/simpleEditor/<?php print $vs_simpleEditor_controller; ?>/Delete/Screen65/object_id/<?php print $vn_subject_id; ?>"
...
<script type="text/javascript">
	...
	function loadSimpleEditorTopForm(ajaxController,screenname,id,form) {
	    ...
	    jQuery.ajax({
	      ...
	      cache: false
	        ...
	        //jQuery('#top_form .sectionBox').delay(5000).columnize({manualBreaks:true});
	    });
	}
	...
	jQuery(document).ready(function(){
		...
		jQuery('div[id*=topform_attribute_165Item]').css('display','block');
	});
</script>
